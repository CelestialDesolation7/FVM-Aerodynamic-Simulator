...
// 用于描述星舰形状的比例顶点
struct starshipPoint
{
    float x;
    float y;
};

constexpr float wingRootRadius = 90.0f;                   // 襟翼根部的半径，无量纲
constexpr starshipPoint wingRootLB = {-45.0f, 77.9422f};  // 襟翼根部的左下顶点，在圆周上
constexpr starshipPoint wingRootRB = {10.9682f, 89.329f}; // 襟翼根部的右下顶点，在圆周上
constexpr starshipPoint wingRootLT = {-6.0f, 112.0f};     // 襟翼根部的左上顶点
constexpr starshipPoint wingRootRT = {6.0f, 112.0f};      // 襟翼根部的右上顶点
constexpr starshipPoint wingLB = wingRootLT;              // 可动襟翼的左下顶点
constexpr starshipPoint wingRB = wingRootRT;              // 可动襟翼的右下顶点
constexpr starshipPoint wingLT = {-3.0f, 186.0f};         // 可动襟翼的左上顶点
constexpr starshipPoint wingRT = {3.0f, 186.0f};          // 可动襟翼的右上顶点
...
struct SimParams
{
    ...
    float obstacle_rotation = 0.0f;                       // 障碍物旋转角度，单位 rad
    ObstacleShape obstacle_shape = ObstacleShape::CIRCLE; // 障碍物形状，枚举值
    float wing_rotation = 0.0f;                           // 仅当obstacle_shape为星舰时有效，表示其襟翼的旋转角度，单位 rad
