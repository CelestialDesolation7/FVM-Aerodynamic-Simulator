...
- **AUSM+ 数值通量**：高精度、低耗散的对流项格式
- **TVD Runge-Kutta**：二阶时间积分，保持总变差递减特性
...
- **GPU**：NVIDIA 显卡，计算能力 ≥ 7.5（RTX 20 系列及以上）
  ...
  - 最低：RTX 2060
- **内存**：8GB RAM（16GB 推荐）
- **显存**：4GB VRAM（512×256 网格约需 100MB）
...
#### 仿真控制
- **开始/暂停**：`Ctrl+Space` 或点击按钮
- **重置仿真**：清空场数据，重新初始化
- **单步执行**：暂停状态下逐步推进

#### 网格设置
- **X 方向网格数**：128 ~ 2048（默认 512）
- **Y 方向网格数**：64 ~ 1024（默认 256）
- **计算域尺寸**：设置物理空间大小（米）

#### 来流参数
- **马赫数**：0.5 ~ 10（默认 3.0，超音速）
- **来流温度**：200K ~ 500K（默认 300K）
- **来流压强**：10kPa ~ 200kPa（默认 101.325kPa）
...
- **值域范围**：手动设置最小/最大值
- **显示速度矢量**：勾选后显示箭头
- **矢量密度**：2 ~ 100（每多少格显示一个箭头）
- **显示障碍物轮廓**：切换边界线显示

#### 性能优化
- **启用 CUDA-OpenGL 互操作**：
  - 勾选：使用双缓冲 PBO，零拷贝传输（推荐）
  - 关闭：传统 CPU 中转模式
- **实时 FPS**：显示当前帧率
- **仿真耗时**：每步计算时间（毫秒）
