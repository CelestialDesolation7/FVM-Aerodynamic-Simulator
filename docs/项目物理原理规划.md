...
- $Pr$：普朗特数（无量纲），空气取 $Pr = 0.72$，反映动量扩散与热扩散的相对快慢
...
其中 $\mathbf{R}_{i,j}$ 是空间离散产生的右端项（通量散度的负值）。本项目使用**一阶显式欧拉法**进行时间推进：

$$
\mathbf{U}_{i,j}^{n+1} = \mathbf{U}_{i,j}^n + \Delta t \cdot \mathbf{R}_{i,j}^n
$$

各符号含义：

- $\mathbf{U}_{i,j}^n$：第 $n$ 个时间步时单元格 $(i,j)$ 的守恒变量
- $\mathbf{U}_{i,j}^{n+1}$：下一个时间步的守恒变量
- $\Delta t$：时间步长（由 CFL 条件确定，见 4.5 节）
- $\mathbf{R}_{i,j}^n$：当前时间步的右端项，即通量散度的负值

显式欧拉法实现简单，且适合 GPU 并行计算（每个单元格独立更新），但对时间步长有严格的稳定性限制（CFL 条件）。
...
### 5.2 有粘性Navier-Stokes方程（工程重构与核函数融合）

在引入双能量法以及高马赫数鲁棒求解后，单纯的“算一遍粘性存回全局内存，再拿出来算速度发散”会遭遇极大的显存带宽瓶颈。因此在当前的最新架构中，**严格应用了 Strang 算子分裂并且实现了寄存器级别的核函数融合**：

在一个时间步 `dt` 中：

```
1. 扩散半步 (dt/2)
    启动融合后的 fusedViscousDiffusionKernel
    对于每个网格点，在寄存器中直接计算且生命周期不泄漏到全局内存：
    -- 1a. 计算八邻居与自身的温度 T
    -- 1b. 计算粘性系数场 mu（Sutherland公式）
    -- 1c. 算出速度空间偏导和散度 -> 算出应力张量 tau_ij 和纯耗散做功推改 Phi
    -- 1d. 直接结合 Fourier 定律算出热通量散度
    最终将 dt/2 时间步长的粘性动量和能量改变增量加到 d_rho_u, d_rho_v, d_E, d_rho_e 重回全局显存。
    
2. 对流全步 (dt)
    2a. 通过新的保守量重算新原始变量（computePrimitivesKernel）
    2b. 计算无粘欧拉通量 (computeFluxesKernel) -> 存入 flux_xxx_X 和 flux_xxx_Y
    	MUSCL重构 → 界面左右状态
        HLLC/HLL黎曼求解 → 界面通量
    2c. 迎风/通量差分更新守恒变量 (updateKernel) 这步会把时间往前推进一整个 dt，并且加入强制正性截断（Density \ Pressure 为负时强力修正）。
    2d. 交换各类缓冲区指针

3. 扩散下半步 (dt/2)
    3a. 对流全步改变了状态，立刻调用 computePrimitivesKernel 刷新供二次扩散使用的参量。
    3b. 再次启动 fusedViscousDiffusionKernel，传入 dt/2 再度计算一次耗散与摩擦。

4. 补齐与同步
    4a. 重新应用各类边界条件 ApplyBoundaryConditionsKernel。
    4b. 全局计算最终呈现的 d_p, d_T 作为本次的收尾动作。
```

> 备注：正因为采用了 Strag 分裂，我们在处理边界的时候实际上只需要处理对于“密度/动量/能量”的纯外推/反推即可，通量与应力的计算在各自的逻辑域内已经完全闭环。
...
**代码实现规则（`DUAL_ENERGY_ETA_THRESHOLD = 0.1f`）：**

在每一步由 `computePrimitivesKernel` 刷新压力速度时，计算内能占比数 $\eta$：
$$ \eta = \frac{E - \frac{1}{2}\rho(u^2+v^2)}{E} $$

1. **高马赫极限分支**：如果 $\eta < 0.1$（说明内能占比太低），直接抛弃减法，**使用平行追踪到的独立 $\rho e$ 获取压力 $p = (\gamma-1)(\rho e)$**，彻底阻断减法误差污染。
2. **正常亚音速/低超音分支**：如果 $\eta \ge 0.1$，优先相信更具能量绝对守恒特征的主路径，依然维持 $p = (\gamma-1)[E - \frac{1}{2}\rho(u^2+v^2)]$。并且在这个安全区域，让辅助内能对齐主路径：将真实的 $E$ 中减算出来的内能同步写入 $\rho e$ 中覆盖并清洗，以防止独立的方程漂移。
